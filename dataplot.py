...
with open('data_check.pkl', 'rb') as infile:
    list_result = pickle.load(infile)

data = list_result
data = np.array(data[:], dtype=object)
print(data[:,1])
mom_in = np.stack(data[:,-6],axis=0)
print(mom_in[:,0])
mom_out = np.stack(data[:,-5],axis=0)

pd.DataFrame(mom_in>mom_out).mean(0)
